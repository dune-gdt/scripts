...
def result_file(exclude, out_fn):
    ...
    with open(out_fn, "wb") as outfile:
        ...
        for procs in [int(math.pow(2, i)) for i in range(MIN_POW, MAX_POW + 1)]:
            ...
            for result_count, sub in enumerate(
                [
                    ...
                    if os.path.isdir(os.path.join(DATADIR, f)) and "-%d." % (procs) in f
                ]
            ):
                with open(os.path.join(DATADIR, sub, "profiler.csv")) as infile:
                    ...
                    out_row = {
                        key: in_row[key] for key in in_row.keys() if key in out_fields
                    }
                    out_row["procs"] = procs
            ...
            if result_count > 0:
                avg_row = {
                    key: "=MITTELWERT({0}{1}:{0}{2})/1000".format(
                        LETTERS[i], out_row_count - result_count - 1, out_row_count - 1
                    )
        ...
        def w_row(base_idx, idx):
            fac_row = {
                key: "={0}{1}/{0}{2}".format(
                    LETTERS[i], mean_rows[base_idx], mean_rows[idx]
                )
                for i, key in enumerate(out_fields)
            }
