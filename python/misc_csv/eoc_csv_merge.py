...
parser.add_option(
    "-r", "--runtype", dest="runtype", default="refine", help="runtype", type="string"
)
parser.add_option(
    "-c",
    "--caption",
...
with open(out_fn, "wb") as out_file:
    ...
    row_count = len(
        list(
            CsvReader(
                open(csv_filenames[0]), delimiter="\t", fieldnames=origin_column_header
            )
        )
    )
    ...
    readers = dict(
        zip(
            ...
            [
                CsvReader(
                    open(fn, "rb"), delimiter="\t", fieldnames=origin_column_header
                )
    ...
    target_csv = CsvWriter(
        out_file, target_column_header, delimiter="\t", extrasaction="raise"
    )
    ...
    for title, reader in readers.iteritems():
        for (c_id, l_row) in zip(range(row_count), reader):
            ...
            for key, value in l_row.iteritems():
                ...
                if t_key not in target_column_header:
                    continue
